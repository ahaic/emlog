...
class sitemap {
	...
	private function setData() {
		...
		//页面
		foreach(Option::get('navibar') as $key => $val) {
			if ($val['hide'] == 'y' || !empty($val['url'])){continue;}
			$lastmod = isset($lastCommentTime[$row['gid']]) ? $lastCommentTime[$row['gid']] : time();
			$data[] = array('url'=>Url::log($key),'lastmod'=>$lastmod,'changefreq'=>$this->changefreq[1],'priority'=>$this->priority[1]);
		}
		//分类
